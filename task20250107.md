...
```
graph TD
    A[PowerApps V2 Trigger] --> B[Parse JSON]
    B --> C[Get Item from SharePoint]
    C --> D[Update Item in SharePoint]
    B --> E[Send Email to Assignee]
    D --> F[Send Email to Assignee]
    D --> G[Error Handling Scope]
    G --> H[Send Error Notification to Admin]
```
